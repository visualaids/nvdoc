...
備註：如果你在 bigfiles.txt 裡面看到有些檔案名稱是亂碼，那麼很可能該檔案名稱有些字元不是繁體中文（例如簡體字）。欲解決此亂碼問題，你可以進入 Windows 控制台的「語言與地區」，找到「系統管理語言設定」，點「變更系統地區設定」，然後把「地區設定」視窗裡面的「使用 Unicode UTF-8 提供全球語言支援」打勾。變更此設定之後，會需要重開機。重開機之後，請再執行一次上面的指令來產生 bigfiles.txt。但請注意，變更此選項之後，可能會導致某些應用程式無法正常顯示中文。
